...
<?php if (get_theme_mod('blog_single_footer_meta', 1)): ?>
  <footer class="entry-meta text-<?php echo get_theme_mod('blog_posts_title_align', 'center'); ?>">
    ...
    if ('' != $tag_list) {
      $meta_text = __('Bài viết này được đăng ở <strong>%1$s</strong> và từ khóa %2$s.', 'flatsome');
    } else {
      $meta_text = __('Bài viết này được đăng ở <strong>%1$s</strong>. Đánh dấu <a href="%3$s" title="Permalink to %4$s" rel="bookmark">liên kết</a>.', 'flatsome');
    }
